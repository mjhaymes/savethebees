...
void drawChar(char letter, unsigned char x, unsigned char y, unsigned char color) {
    unsigned char j,k;
    for (j=0; j<5; j++) {
        for (k=0; k<8; k++) {
            if ( ( (ASCII[letter-0x20][j]>>k)&1 ) == 1) {
                ssd1306_drawPixel(x+j, y+k, color);
            } else {
...
void drawString(char message[50], unsigned char x, unsigned char y, unsigned char color) {
    unsigned char s;
    for (s=0; s<strlen(message); s++) {
        drawChar(message[s],x,y,1);
